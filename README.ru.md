...
- **Сборка** — **Build Game** в один клик под все семь платформ: подготовленный контент в сжатых zstd архивах `IcePak` за виртуальной файловой системой, манифесты с SHA-256, NSIS `.exe` и WiX `.msi`, `.deb` и `.AppImage`, macOS `.dmg` / `.pkg` с подписью кода и опциональной нотаризацией, Android `.apk` / `.aab`, iOS `.ipa`, раскладки Xbox с `MicrosoftGame.config` и пакеты `.msixvc` / `.xvc` через Microsoft GDK, DLC-паки, монтирующиеся поверх базовой игры, и headless-серверы. **Редактор на Android** собирает подписанные `.apk` прямо на телефоне, без участия ПК.
...
| **Linux** | ✅ | ✅ |
| **macOS** | ✅ | ✅ |
| **Android** | ✅ *(только игры под Android)* | ✅ |
| **iOS** | ❌ | ✅ |
| **Web** | ❌ | ✅ |
| **Xbox** *(Microsoft GDK)* | ❌ | ✅ |

Игры под Xbox собираются **только на Windows** — Microsoft GDK, его интеграция с MSVC и
`MakePkg` существуют исключительно под Windows. Выбор Xbox на хосте Linux или macOS
сообщает, что нужна машина на Windows, — ровно так же, как macOS и iOS сообщают, что им
нужен Mac.

**Android — это и платформа разработки.** Движок поставляется Android-приложением, в котором
лежат лаунчер, полноценный редактор и игровой рантайм, так что телефон или планшет делает
игры сам по себе. По сути это та же настольная установка — тот же лаунчер, тот же редактор,
те же панели, тот же Lua-скриптинг и тот же пайплайн ассетов — с четырьмя осознанными
отличиями:

- **Build Game собирает только под Android.** Выбор целевой платформы зафиксирован на
  Android: кросс-сборка бинарников под Windows, Linux, macOS, iOS, Web или Xbox требует
  тулчейнов, которых на телефоне нет. Всё остальное в окне Build Game работает как на
  десктопе.
- **Апдейтера нет.** Android-приложения обновляются через магазин или установкой `.apk`,
  поэтому апдейтер туда не собирается, а лаунчер прячет его кнопку.
- **Remote Preview нет.** Он транслирует вьюпорт редактора *на* Android-устройство; на самом
  устройстве режим Play и есть этот предпросмотр.
- **Нет Python-скриптинга.** CPython не собирается под Android в наборе зависимостей движка,
  поэтому редактор на Android — это Lua и визуальный скриптинг. Собранные из него игры ведут
  себя точно так же: Python — инструмент редактора, а не рантайма.

Сборка на устройстве упаковывает движок, ваш контент, ваши Lua-скрипты и ассеты плагинов и
модов. Интеграции Play/AdMob/Firebase и формат бандла `.aab` создаёт сборка Gradle, и им
по-прежнему нужна настольная установка; редактор на Android прямо пишет об этом в окне Build
Game, а не предлагает опции, которые не сможет выполнить.

Проект переезжает между телефоном и ПК одним архивом. **Build Game → Экспорт проекта → Экспорт
проекта (.zip)** упаковывает весь открытый проект в `IceBoxExports/` и в папку **«Загрузки»**
телефона — распакуйте на ПК и продолжайте там; **Импорт проекта (.zip)…** в лаунчере
распаковывает такой архив обратно в `IceBoxProjects/`, проверяет, что внутри действительно есть
`.iceproject`, и добавляет проект в список.

---

## 🏗️ Архитектура

IceBoxEngine состоит из нескольких компонентов:

| Компонент | Бинарник | Описание |
|-----------|--------|-------------|
| **Launcher** | `IceBoxEngineLauncher` | Точка входа для пользователей. Управляет проектами (создание, открытие, удаление), проверяет обновления движка и запускает редактор для выбранного проекта. |
...
| **Updater** | `IceBoxEngineUpdater` | Отдельное приложение обновления. Проверяет манифест обновлений, затем скачивает, проверяет и устанавливает новую версию движка — всегда по вашему явному подтверждению, никогда молча — и после установки открывается снова, чтобы сообщить результат. |
| **Runtime** | `IceBoxEngineRuntime` | Легковесный исполняемый файл без редактора, поставляемый со собранными играми. Запускает игровой проект напрямую на целевой платформе. |

На **Android** лаунчер и редактор — это один процесс внутри одного APK: сначала открывается
экран лаунчера, и выбранный проект передаётся редактору напрямую, без запуска второго
исполняемого файла. Апдейтер под Android не собирается вовсе.

---

## ✅ Статус проекта

**IceBoxEngine вышел в релиз.** Текущая стадия — **Release 1.0**: лаунчер, редактор, апдейтер и рантайм стабильны, и на движке можно собирать и выпускать полноценные коммерческие игры на всех поддерживаемых платформах.

Релиз — не значит «закончен». Движок таким и не бывает: новые возможности, новые инструменты и постоянные улучшения продолжают выходить, и каждое из них приходит к вам бесплатным обновлением. На версии 1.0 изменилось другое — то, как они приходят: как дополнения поверх стабильной основы, а не как почва, уходящая из-под проекта, который вы уже на нём строите.
...
### Движок и редактор — Android

| | |
|-|-|
| **ОС** | Android 8.0+ (API 26). ABI редактора: `arm64-v8a` (телефоны и планшеты), `x86_64` (эмуляторы, Chromebook), `armeabi-v7a` и `x86` (32-битные устройства) |
| **ОЗУ** | 4 ГБ (рекомендуется 6 ГБ — редактор держит весь проект в памяти) |
| **GPU** | Vulkan 1.1-1.4 либо OpenGL ES 3.2 с автоматическим откатом на OpenGL ES 3.0 |
| **Диск** | 2 ГБ свободно — при первом запуске приложение распаковывает данные движка. Проекты, сборки, экспорт и ключи лежат в `/storage/emulated/0/IceBoxEngine/`, как только приложению выдан доступ ко всем файлам, — туда добираются и файловый менеджер, и ПК по USB |
| **Ввод** | Сенсор; клавиатура и мышь по USB или Bluetooth тоже работают и обрабатываются как обычный ввод |
...
| | |
|-|-|
| **Устройство** | Xbox Series X\|S и Xbox One (`Gaming.Xbox.*`) либо Windows 10+ x64 через Microsoft Store / приложение Xbox (`Gaming.Desktop.x64`) |
| **GPU** | Direct3D 12 — единственный графический API, через который выводит тайтл на Xbox |
...
| 🤖 **Android** | Android SDK 37+, NDK 29+, Java JDK 25+, Gradle 9.7.0 *(скачивается автоматически)*. В **редакторе на Android** ничего дополнительно не нужно — он перепаковывает шаблон рантайма, лежащий внутри APK, и сам подписывает результат |
| 🌐 **Web** | [Emscripten SDK](https://emscripten.org/) — плюс Node.js 24+, если собираете модель памяти `wasm64` |
| 🎮 **Xbox** | **Только хост Windows.** [Microsoft GDK](https://github.com/microsoft/GDK/releases) (его установщик задаёт `GameDK`, `GameDKLatest`, `GRDKLatest`) плюс Windows 11 SDK с Direct3D 12 и DirectX Shader Compiler. Семействам **Xbox One** и **Xbox Series** дополнительно нужны закрытый **GDKX** из Partner Center (ID@Xbox / Xbox Managed, задаёт `GXDKLatest`) и девкит для запуска; их триплеты vcpkg и тулчейн-файлы GDK поставляются вместе с движком в `Tools/BuildSystem/Utilities/`. Семейству **PC** (`Gaming.Desktop.x64`) не нужно ничего сверх публичного GDK, и оно переиспользует те же зависимости `x64-windows`, что и цель Windows |
...
Запустите **IceBoxEngineLauncher** — с ярлыка на рабочем столе, из меню «Пуск» / меню приложений или файлом `IceBoxEngineLauncher` в папке установки. На ещё не активированном компьютере лаунчер откроет экран активации: вставьте лицензионный ключ и нажмите **Активировать**. Это разовый шаг для каждого компьютера — после него лаунчер, редактор и апдейтер открываются сразу.
...
### Инструменты сборки Xbox (Windows)

> Сборки под Xbox идут **только на хосте Windows**. Microsoft GDK, его интеграция с MSVC и
> `MakePkg` существуют лишь под Windows, а кросс-компилятора под `Gaming.Desktop.x64` или
> `Gaming.Xbox.*` нет. `build_xbox.sh` на Linux/macOS — заглушка, которая именно это и
> сообщает.

```bash
# 1. Поставить публичный Microsoft GDK — выберите релиз, совпадающий с вашими
#    Visual Studio и Windows SDK: https://github.com/microsoft/GDK/releases
#    Установщик задаёт GameDK, GameDKLatest и GRDKLatest. После установки откройте
#    новую консоль, чтобы сборка их увидела.

# 2. (Только для консольных семейств) Поставить закрытый GDKX из Partner Center.
#    Microsoft выдаёт его зарегистрированным участникам ID@Xbox и партнёрам Xbox
#    Managed под NDA; он задаёт GXDKLatest, и именно против него собираются
#    Gaming.Xbox.XboxOne.x64 и Gaming.Xbox.Scarlett.x64. Публичный GDK несёт
#    только половину Gaming.Desktop.x64
```

Сборка пишет **раскладку игры Xbox** — саму игру плюс `MicrosoftGame.config` и арты
магазина в `Images/`, — а по запросу пакует её в `.msixvc` (PC) или `.xvc` (консоль) через
`MakePkg`. Раскладку для PC регистрируют и запускают через `wdapp register`, консольную
разворачивают через `xbapp deploy`. Всё, что показывает диалог — семейство устройств,
идентификатор пакета, ID издателя, Title ID, Store ID, сеть Xbox, — описано в
**[Профайлинг и сборка игр → 8.7 Xbox](Documentation/RU/Profiling-And-Building-RU-DOC.md#87-xbox-microsoft-gdk)**.
...
| [Профайлинг и сборка игр](Documentation/RU/Profiling-And-Building-RU-DOC.md) | Профайлеры, статистика, сборка под все семь платформ, инсталляторы, DLC, headless-серверы. |
...
| [Python API](Documentation/RU/PythonAPI-RU-DOC.md) | Автоматизация редактора и инструментарий: сущности, уровни, панели, лог консоли, мост в игровую ВМ Lua, конфигурация проекта, проверка ассетов, установка пакетов с PyPI через pip и автономный запуск редактора из командной строки. |
...
**IceBoxEngine** — проприетарное программное обеспечение.  
