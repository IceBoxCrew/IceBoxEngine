...
- **Качество картинки** — FXAA / MSAA / SSAA, масштаб рендера 1-200%, апскейл **FSR** и **NIS** с пресетами качества, вывод **HDR10**, VSync, режим низкой задержки и **адаптивное качество**, которое само удерживает целевой FPS.
- **Сцены и ECS** — Ядро на основе Entity-Component-System (EnTT), аутлайнер уровней, переиспользуемые классы сущностей с наследованием, редактор свойств/мира и **асинхронная загрузка уровней** с прогрессом для экранов загрузки.
- **2D-физика** — Твёрдые тела, коллайдеры и сочленения на базе Box2D с многопоточным решателем (enkiTS) на десктопе и мобильных, платформы с проходом снизу, рэгдоллы и физика костей, коллизии прямо из спрайтов, флипбуков и тайлмапов и **разрушение** на настоящие физические осколки.
...
- **Геймплейные системы** — Пулы объектов, волны врагов, кулдауны, достижения, твины, конечные автоматы, таймеры, корутины, шина событий, сидированные потоки случайных чисел для детерминизма и самосохраняющиеся таблицы.
...
- **Ассеты** — 25 типов ассетов, у каждого свой редактор, скрытые сайдкары с настройками импорта, редиректоры, просмотр ссылок, массовое редактирование, импорт из Aseprite и GIF и **подготовка ассетов** (WebP / KTX2 / Opus / Vorbis / VP9 / сабсеттинг шрифтов) с упаковкой в `IcePak` за виртуальной файловой системой.
...
- **Инструментарий** — Встроенный профилировщик Tracy, профайлер кадра с записью трейсов, GPU-таймингами по проходам, учётом памяти и VRAM и детектором фризов, оверлеи статистики, консоль разработчика с командами и переменными (CVar), **Remote Preview** на Android-устройство по USB, справочник горячих клавиш и пайплайн сборки игры **Build Game** в один клик для всех поддерживаемых платформ.
