...
| **Signing** | Automatic, and checked before the build starts. If no keystore is set when you press **Build**, the editor creates a development one (PKCS#12, RSA-2048, 30 years) in `IceBoxKeystores/`, fills the Signing fields with it and keeps building — the same thing a desktop build does with the debug key, so an APK is always installable. Passwords are never written to disk, so the password box is empty again in the next session: a keystore the editor created itself is reopened on its own, while a keystore whose password you chose wants that password typed again before the build. **Generate keystore…** makes one up front, and a keystore made by `keytool` on a PC works too, as long as it is PKCS#12 — JDK 9+ writes that by default; the old JKS format cannot be read on Android at all, so convert it first with `keytool -importkeystore -srckeystore key.jks -destkeystore key.jks -deststoretype pkcs12`. If the keystore cannot be opened the build stops right there and the log names the reason, instead of failing minutes later at the signing step. Use your own keystore for anything you publish |
