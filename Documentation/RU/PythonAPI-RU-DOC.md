...
### Что такое Python API в IceBoxEngine?

**Python API** — это скриптовый интерфейс для **редактора** IceBoxEngine. В отличие от Lua API, который используется для игровой логики (скрипты объектов, виджеты, кинематики), Python API предназначен для:
...
Откроется панель с заголовком **Консоль Python** — главное рабочее место для всего, что описано в этом руководстве. Здесь вы пишете и запускаете Python, вставляете готовые сниппеты, сохраняете и загружаете файлы скриптов и читаете встроенный справочник API. На практике почти вся повседневная работа с Python в IceBoxEngine происходит именно в этом окне.
...
> Этот раздел — краткий, но полный справочник по базовому Python. Если вы уже знаете Python — смело переходите к [разделу 4](#4-модули-api--обзор). Если нет — прочитайте этот раздел, и вы сможете уверенно писать скрипты в Python-консоли IceBoxEngine.
...
Строки — один из самых используемых типов. В консоли IceBoxEngine вы постоянно работаете со строками (имена сущностей, пути к файлам, компоненты).
...
Кортеж — как список, но **неизменяемый**. Многие функции IceBoxEngine API возвращают кортежи (например, позиция `(x, y, z)`).
...
# Распаковка из функций IceBoxEngine API
...
Словарь — коллекция пар «ключ: значение». IceBoxEngine API активно использует словари для передачи данных компонентов.
...
#### Использование с IceBoxEngine API
...
#### Практические примеры с IceBoxEngine
...
#### Практические функции для IceBoxEngine
...
# Применение с IceBoxEngine таймерами
...
Python имеет обширную стандартную библиотеку. Самые полезные модули для работы в IceBoxEngine:
...
Снимает вьюпорт игры в PNG и возвращает путь, по которому файл будет записан,
либо пустую строку, если запрос отклонён.

Захват происходит **в конце текущего кадра**, а не внутри вызова: в момент
работы вашего скрипта нужного кадра ещё не существует. Поэтому файл появляется
кадром позже — опрашивайте `editor.is_screenshot_pending()` или возвращайтесь по
таймеру, прежде чем читать его.

Снимается только вьюпорт игры. Панели, тулбар и консоль в кадр не попадают
никогда, поэтому снимок из автоматизации редактора совпадает с тем, что выдаст
собранная игра.

Пути здесь **не** ограничены песочницей — это инструмент разработчика.
Относительный путь считается от папки проекта; абсолютный используется как есть;
пустой путь пишет файл с меткой времени в `<проект>/Screenshots`. Недостающие
папки создаются, а `.png` дописывается, если расширения нет.

```python
# с меткой времени, в <проект>/Screenshots
path = editor.screenshot()

# с именем, относительно проекта
...
Одновременно в очереди только один захват. Второй запрос в том же кадре вернёт
`''` и напишет предупреждение, а не подменит первый.

#### `editor.is_screenshot_pending()` → `bool`

`True`, пока поставленный в очередь захват ещё не записан.

#### `editor.get_last_screenshot()` → `str`

Абсолютный путь последнего попытанного снимка. Пустая строка, пока снимков не
было.
...
def shoot_then_check():
    ...
    if not target:
        editor.log_error('снимок отклонён')
        return

    def verify():
        if editor.is_screenshot_pending():
            editor.set_timer(0.1, verify)     # ещё не записан, вернёмся позже
            return
        last = editor.get_last_screenshot()
        editor.log_info(f'{last} существует: {os.path.exists(last)}')
...
> **См. также:** игры снимают собственный экран через таблицу `Screenshot` в Lua
> API, где пути ограничены записываемой областью сохранений.
...
Обновляет браузер контента (обычно обновляется автоматически).
...
`show_grid` — фактическая видимость сетки главного вьюпорта: настройка **Показывать сетку вьюпорта** И тумблер сетки на тулбаре. Значение `False`, если выключено хотя бы одно из них. У классового редактора свой тумблер сетки, он хранится внутри ассета `.ice_class` и здесь не отражается.
...
Этот подраздел покрывает тул-слой Python: запуск скрипт-файлов с диска, управление автодополнением консоли и интеграцию с меню **Запустить Python-скрипт** панели «Консоль Python», которое работает из папки `Tools/PythonScripts/`.

> 📁 **Расположение скриптов (только для редактора — НЕ игровой контент):**
> - `Tools/PythonScripts/` — пользовательские тул-скрипты, рекурсивное сканирование. Каждая подпапка становится **категорией** (подменю) в меню *Запустить Python-скрипт* этой панели.
...
> Это та же точка входа, что используют пункты меню **Запустить Python-скрипт → <категория> → <скрипт>** этой панели.
...
## 8. Модуль `browser` — Браузер контента

Модуль `browser` позволяет программно управлять файлами и папками проекта через браузер контента.

### 8.1 Навигация

#### `browser.get_current_dir()` → `str`

Возвращает текущую директорию в браузере контента.
...
Возвращает пути выделенных элементов в браузере контента.
...
Выделяет элемент в браузере контента.
...
Пересобирает вид браузера контента. Вызывайте после создания, удаления или перемещения файлов на диске из Python, чтобы панель увидела изменения.
...
`True`, пока панель браузера контента имеет фокус клавиатуры.

```python
if browser.is_focused():
    editor.log_info('Браузер контента в фокусе')
```
...
A: В редакторе IceBoxEngine откройте её через **`Инструменты → Запустить Python-скрипт`**. Полный обзор панели — в разделе [Окно «Запустить Python-скрипт»](#окно-запустить-python-скрипт).
